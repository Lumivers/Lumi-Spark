...
| **阶段 7：敌人 AI 与行为树系统** | AIController, EnemyBase, 7类敌人, 掩体, BT 节点, Boss | **已完成** | **100% ✅** |
| **阶段 8：驱动核心与装备系统** | 6 槽驱动核心, 驱动盘, 套装判定, 属性汇总管线 | **已完成** | **100% ✅** |
...
### 6.3 武器特化派生类（5 种）
- [x] **步枪 (`ALSRifle`)**：全自动/半自动模式切换、标准弹道
- [x] **冲锋枪 (`ALSSMG`)**：高射速低伤害、近距优势散布曲线
- [x] **霰弹枪 (`ALSShotgun`)**：8 弹丸独立锥形散布与独立射线检测
- [x] **狙击枪 (`ALSSniperRifle`)**：蓄力增伤、3.0x 爆头倍率、开镜极窄 FOV
...
### 7.3 七类敌人行为特征
- [x] **近战杂兵 (`ALSEnemy_Melee`)**：直线冲锋蓄力、近距离挥砍打断破招
- [x] **远程步枪兵 (`ALSEnemy_Ranged`)**：掩体对射、三连发点射、贴近（<6米）战术后撤
- [x] **红外狙击手 (`ALSEnemy_Sniper`)**：远距站位、1.5s 动态红色瞄准激光预警线
- [x] **防弹盾兵 (`ALSEnemy_Shielder`)**：正面 120° 扇区绝对免疫物理子弹、逼迫滑铲绕后/元素破盾
- [x] **自爆兵 (`ALSEnemy_Bomber`)**：650 极速冲刺、近身/死亡高爆元素 AoE 连锁自爆
- [x] **元素精英怪 (`ALSEnemy_Elite`)**：复合多层元素盾、固有免击退霸体、周期性范围元素战技
- [x] **多阶段 Boss (`ALSEnemyBoss` + `FLSBossPhase`)**：70%/30% 血量门槛转场、转场无敌状态机、独立行为树动态热切换、狂暴加成
...
### 8.1 驱动核心数据结构
- [x] **驱动盘结构体 (`FLSDriveDisc`)**：主词条（固定）+ 4 条随机副词条 + 品质等级（绿/蓝/紫/金）
- [x] **6 槽位装载 (`FLSDriveCoreLoadout`)**：1~3 号位基础攻防、4~6 号位流派特化
- [x] **驱动盘数据资产 (`ULSDriveDiscDataAsset`)**：套装词条池、属性区间、掉落权重

### 8.2 套装系统
- [x] **套装激活判定**：4+2 件套或 2+2+2 件套激活效果
- [x] **套装效果引擎**：双暴增幅、精通反应加成、元素伤害提升、充能/缩短 ICD 等

### 8.3 属性计算管线
- [x] **角色最终属性汇总**：基础属性 + 武器加成 + 驱动核心 6 槽叠加 → 输入 `LSDamageCalculator`
