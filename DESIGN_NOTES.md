...
## 32. 三人小队重构与切人即切枪体系：单武器瘦身、顺逆轮切与默认TPS视角适配

### 32.1 为什么要写这段代码（架构角色与交互痛点）
- **痛点解决**：在“FPS 射击 + 原神多角色元素反应”的复合玩法下，传统单人射击的主副双枪（1/2 键切枪、滚轮切枪）与多角色切换（Tab 键切人）会产生严重的按键竞争与心智超载。玩家在第一/第三人称激战中不仅要走位、射击、丢手雷，还要在两把武器与两到三个角色间来回倒手，极易误操作。
- **架构角色**：推行「三人小队，切人即切枪」后，每个角色绑定一把专属元素武器，按键统一为数字键 1/2/3 直切、滚轮顺逆切与 Tab 轮切。一次操作同时完成 **角色模型 + 专属枪械 + 战术技能 + 元素属性** 的整体流转。

### 32.2 为什么这么设计（类职责划分与方案对比）
- **`LSWeaponComponent` 单一职责化（瘦身）**：
  - *旧方案*：武器组件既管生成、又管两把枪在角色手部与背部的插槽挂载（`HandSocket` / `HolsterSocket`）、还管理收拔枪计时器、副武器网络同步与切枪 RPC。
  - *新方案*：武器组件剥离“背包与切枪管理器”职责，只保留唯一的 `CurrentWeapon`，退回到纯粹的“单武器射击与装弹控制器”。换枪行为完全由更高层级的 `LSTeamSwitchComponent` 换人实现，消除了状态重叠。
- **环形轮换寻址（Circular Cycle & 存活校验）**：
  - 滚轮顺逆切与 Tab 键轮切采用 `(ActiveIndex + Step + Total) % Total` 环形寻址，并在遍历时调用 `CanSwitchToIndex` 检查角色是否存活（`!Char->IsDead()`）。若小队某成员阵亡，自动顺延跳过，防止切出尸体崩溃。

### 32.3 UE 框架契合度
- **CDO 与初始摄像机参数**：在 `LSCharacterBase` 的构造函数中直接修改 `SpringArm` 的 CDO 默认值（臂长 300、右肩偏移、开启防穿墙），保证加载完成的第 0 帧即为规范的第三人称视角，消除了在 `BeginPlay` 动态插值造成的镜头拉扯感。
- **Enhanced Input Axis1D 复用**：鼠标滚轮通过增强输入配置为 `Axis1D`（向上 +1.0，向下 -1.0），单一动作通道即可驱动正向与逆向切人，大幅节省 Controller 的输入通道。

### 32.4 游戏开发特有的注意事项
- **视觉穿帮规避（第一人称悬浮手臂）**：UE 的第一人称手臂通常是通过 `SetOnlyOwnerSee(true)` 附加在摄像机上的。在第三人称模式下（摄像机后退 300 距离），如果不显式调用 `ArmsMesh->SetVisibility(false)`，镜头正前方会悬浮一双断臂。在 `UpdateMeshVisibility` 中增加了严格的显隐分支。
- **网络带宽优化**：`LSWeaponComponent` 砍掉了 `PrimaryWeapon`、`SecondaryWeapon`、`CurrentSlot` 3 个属性复制，仅同步唯一的 `CurrentWeapon` 指针，降低了多人联机时的网络带宽消耗。
- **性能零空跑**：`LSTeamSwitchComponent` 构造时将 `bStartWithTickEnabled = false`，仅在换人产生 1.0s CD 倒计时期间短暂使能 Tick，归零后立即自我休眠，平时 CPU 开销为 0。
